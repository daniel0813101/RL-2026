...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            q_target = reward + self.gamma * mask * target_v

        q_1_pred = self.qf_1(state, action)
        q_2_pred = self.qf_2(state, action)
        qf_1_loss = F.mse_loss(q_1_pred, q_target)
        qf_2_loss = F.mse_loss(q_2_pred, q_target)
        ...
        v_target = (q_new_action - alpha.detach() * log_prob).detach()
        ...
        if self.total_step % self.policy_update_freq == 0:
            # actor loss
            ########## Your Code (<5 lines)##########
            actor_q = self.qf_1(state, new_action)
